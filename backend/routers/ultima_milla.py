...
@router.get("/bodegas/{bodega}/domiciliarios", response_model=List[schemas.DomiciliarioStats])
def get_domiciliarios(
    bodega: str,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user)
):
    """Obtiene lista de domiciliarios de una bodega que tienen pedidos pendientes"""
    if current_user.rol not in ["auditor", "administrador"]:
        raise HTTPException(status_code=403, detail="Solo auditores pueden acceder")
    
    # Solo obtener domiciliarios que tengan al menos un pedido pendiente
    domiciliarios = db.query(
        ...
        models.PedidoUltimaMilla.estado == 'pendiente'  # Solo pedidos pendientes
    ).group_by(
        models.PedidoUltimaMilla.documento_domiciliario,
        models.PedidoUltimaMilla.nombre_domiciliario,
        models.PedidoUltimaMilla.bodega
...
@router.get("/domiciliarios/{documento}/pedidos", response_model=List[schemas.PedidoResumen])
...
def iniciar_auditoria_ultima_milla(
    ...
    # Buscar o crear ubicación de origen (bodega)
    ubicacion_origen = db.query(models.Ubicacion).filter(
        models.Ubicacion.nombre == request.bodega
    ).first()
    
    if not ubicacion_origen:
        ubicacion_origen = models.Ubicacion(
            nombre=request.bodega,
            tipo='sede',
            creado_por=current_user.id
        )
        db.add(ubicacion_origen)
        db.flush()
    
    # Buscar o crear ubicación de destino (cliente)
    ubicacion_destino = db.query(models.Ubicacion).filter(
        models.Ubicacion.nombre == 'Cliente'
    ).first()
    
    if not ubicacion_destino:
        ubicacion_destino = models.Ubicacion(
            nombre='Cliente',
            tipo='sede',
            creado_por=current_user.id
        )
        db.add(ubicacion_destino)
        db.flush()
    
    # Asignar ubicaciones a la auditoría
    auditoria.ubicacion_origen_id = ubicacion_origen.id
    auditoria.ubicacion_destino_id = ubicacion_destino.id
...
def limpiar_pedidos_pendientes(
    ...
    if pedido_ids:
        ...
        productos = db.query(models.ProductoPedidoUltimaMilla.id).filter(
            models.ProductoPedidoUltimaMilla.pedido_id.in_(pedido_ids)
        ).all()
        producto_ids = [p.id for p in productos]
        
        # 1. Eliminar novedades de productos
        if producto_ids:
            db.query(models.NovedadProductoUltimaMilla).filter(
                models.NovedadProductoUltimaMilla.producto_id.in_(producto_ids)
