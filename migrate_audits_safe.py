"""
Script para migrar SOLO auditorias y productos desde Render (version segura)
"""
import os
...
try:
    with new_engine.connect() as new_conn:
        ...
        # Usar TRUNCATE CASCADE es mas rapido y seguro
        new_conn.execute(text("TRUNCATE TABLE auditorias CASCADE"))
        new_conn.commit()
        print("   Limpiado\n")
    ...
    with old_engine.connect() as old_conn, new_engine.connect() as new_conn:
        ...
        for row in rows:
            try:
                ...
                count += 1
                if count % 10 == 0:
                    print(f"   Progreso: {count}/{len(rows)}")
            except Exception as e:
                print(f"   Error en auditoria {row[0]}: {e}")
        ...
        for row in rows:
            try:
                ...
                print(f"   Error en producto {row[0]}: {e}")
